...
def doc_analyst_node(state: AgentState) -> Dict:
    """Collects evidence from the PDF report using Gemini RAG."""
    # pdf_path = state.get("pdf_path")
    # if not pdf_path or not Path(pdf_path).exists():
    #     return {"evidences": {"theoretical_depth": [Evidence(goal="Analyze depth", found=False, location="Missing", rationale="No PDF", confidence=1.0)]}}
    ...
    # depth_ev = Evidence(goal="Theoretical Depth", found=found_depth, location=pdf_path, content=depth_answer[:500], rationale=depth_answer[:200], confidence=0.9)
    
    # # Citation Check
    # paths = extract_file_paths(content)
    # hallucinated = [p for p in paths if not Path(p).exists()]
    
    # accuracy_ev = Evidence(goal="Citation Check", found=len(hallucinated) == 0 and len(paths) > 0, location=pdf_path, rationale=f"Verified {len(paths)-len(hallucinated)} paths. Hallucinated: {hallucinated}", confidence=0.8)

    return {"evidences": {"theoretical_depth": [], "report_accuracy": []}}
